...
def load_data(
    ...
    """
    Loads and preprocesses M3 data.
    ...
    data_file_path = ("./data/processed/M3_" +
                      subset +
    ...
    if (test_mode_nrows==None) and (os.path.exists(data_file_path + "_train_set.pt")):
        ...
        # Load raw data
        dataset = pd.read_csv(f"{data_path}/{subset}_clean.csv", nrows=test_mode_nrows, sep=None, decimal=',', engine='python')
        dataset.set_index('Series', inplace = True)
...
#         data_path="/teamspace/studios/this_studio/data/raw",
#         backcast_length_multiplier=8,
#         # test_mode_nrows=100,
#         batch_size=512,
#         forecasting_origin_range_multiplier=1e6) # 2000
# for loop_id in range(len(train_dataloader)):
#     x_train, y_train = next(iter(train_dataloader))
#     bincount_list.append(x_train['groups'].squeeze().bincount(minlength=1428).numpy())
# stacked_bincounts = np.stack(bincount_list,0)
# np.mean(stacked_bincounts, 0)/512 # should all equal 1/1428
