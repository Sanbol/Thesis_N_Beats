...
RESULTS_FILE = BASE_DIR / "nhits_experiment_results_tuned.csv"
...
def print_summary(results):
    ...
    print(f"\n{'='*90}")
    print(f"  N-HiTS-S TUNED RESULTS SUMMARY (hidden=512, blocks=10)")
    print(f"{'='*90}")
...
def main():
    ...
    print(f"\n{'#'*70}")
    print(f"  N-HiTS-S TUNED EXPERIMENT RUNNER")
    print(f"  Architecture: hidden=512, n_blocks=10 (M4 HP-tuned)")
    print(f"  Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"  Total experiments: 24 (6 scratch + 6 pretrain + 6 finetune + 6 zeroshot)")
    print(f"{'#'*70}")

    shutil.copy2(MAIN_PY, MAIN_PY_BACKUP)
    print(f"\n  Backed up main_nhits.py -> main_nhits_backup_tuned.py")
